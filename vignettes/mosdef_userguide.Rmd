...
RMDs are a great way of handing over reports to colaborators. These functions are supposed to improve the report quality by upgrading the report turning normal tables into interactive tables linking to further websites.
...
You can use acces all of these easily by using one function
```{r}
...
```
Important to know is that the buttonifier function returns a DT:datatable not a df. This is to ensure that the excape = FALSE parameter of datatable is set and not forgotten as the links/buttons will not work otherwise.
Advanced users that want further customisation options to their datatable can ensure a dataframe is returned (then the escape = FALSE must be set by hand):
```{r}
res_subset <- deseqresult2df(res_airway, FDR = 0.05)[1:500, ]
...
All of the functions included inside the buttonifier function are also avaiulable as singular functions in case you are only interested in one of them:
```{r}
res_subset <- deseqresult2df(res_airway, FDR = 0.05)[1:500, ]
