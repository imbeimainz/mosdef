...
* topGO `r BiocStyle::Biocpkg("topGO")`
* goseq `r BiocStyle::Biocpkg("goseq")`
* clusterProfiler `r BiocStyle::Biocpkg("clusterProfiler")`
...
res_enrich_macrophage_topGO <- topGOtable(
  ...
  geneID = "symbol", # Which format the genes are provided.
  # If you provide a res_de and a dds mosdef does it for you and uses symbols.
  # If you provide vectors please specify
  de_type = "up_and_down", # which genes to analyse.
  # Default is all ("up_and_down")
  ...
  topGO_method2 = "elim", # Character, specifying which of the methods implemented by topGO
  # Default is elim
  ...
  top_de = 700, # number of genes to be considered in the enrich analysis
  # Default is all, can be reduced to reduce redundancy.
  # In this case Takes the top 700 highest DE genes based of padj score.
  # If this number is bigger than the total amount of de genes the parameter defaults to all genes.
...
* geneID: Which format the genes are provided. If you provide a `DESeqDataset` and `DESeqResults`
  mosdef does it for you and uses symbols. If you provide vectors please specify.
...
clupro_macrophage <- cluproTable(
  ...
  dds = dds_macrophage,
  mapping = "org.Hs.eg.db",
  keyType = "SYMBOL" # Important for the enrichGO function that is run later on.
  # If using dds and res_de has to be "SYMBOL" which is also the default
)
...
volcPlot <- de_volcano(
  ...
  L2FC_cutoff = 1 # Where to draw the lines in the plot and which genes to mark
  # as significant default is one (meaning L2FC +/-1)
)
volcPlot
```
...
Volc_GO <- go_volcano(
  ...
  enrich_col = "genes", # columnname of the column in your res_enrich containing the gene symbols
  # For an example see topgotable data provided in mosdef: data(res_enrich_macrophage_topGO, package = "mosdef")
...
maplot <- plot_ma(res_macrophage_IFNg_vs_naive_subset,
...
```{r}
maplot_genes <- plot_ma(res_macrophage_IFNg_vs_naive_subset,
                        FDR = 0.1,
                        intgenes = c(
                          "SLC7A2",
                          "CFLAR",
...
buttonifier(res_subset,
            ...
            ens_col = "id", # where to find the ensemble IDs
            ens_species = "Homo_sapiens" # The species you are working on
)
```
